...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: scheduledOccurrences } = await supabase
      ...
      .select('id, metricool_post_ids, client_id, caption, platform_captions, scheduled_at')
      .eq('status', 'scheduled')
    ...
    for (const occ of scheduledOccurrences || []) {
      ...
      const networkToPostId = occ.metricool_post_ids || {}
      const networks = Object.keys(networkToPostId)
      if (networks.length === 0) continue

      try {
        // The GET-single-post endpoint wraps its response in {data: ...},
        // unlike the list endpoint used earlier in this file, which
        // returns the array directly -- confirmed live the hard way (this
        // was silently reading `undefined` providers before being fixed).
        const schedulerStatusByNetwork = {}
        const uniquePostIds = [...new Set(Object.values(networkToPostId))]
        for (const postId of uniquePostIds) {
          ...
          for (const provider of post.providers || []) {
            schedulerStatusByNetwork[provider.network] = provider.status
          }
        }

        const statuses = Object.values(schedulerStatusByNetwork)
        ...
        const allPublishedPerScheduler = statuses.every(s => s === 'PUBLISHED')
        if (!allPublishedPerScheduler) continue // still PENDING somewhere -- check again next cycle

        // Metricool's own status is not sufficient evidence on its own --
        // confirmed live: a real Instagram post came back PUBLISHED with
        // detailedStatus "Published", using real normalized media, and
        // hours later had zero corresponding post in Instagram's own
        // analytics feed. Facebook's status was accurate in the same
        // test, so this cross-checks every network the same way rather
        // than assuming the gap was Instagram-specific -- every network
        // needs its own real post found before this occurrence is trusted
        // as actually live.
        const scheduledDate = new Date(occ.scheduled_at)
        const windowStart = new Date(scheduledDate.getTime() - 24 * 60 * 60 * 1000).toISOString()
        const windowEnd = new Date(scheduledDate.getTime() + 24 * 60 * 60 * 1000).toISOString()
        const unconfirmedNetworks = []

        for (const network of networks) {
          const captionText = occ.platform_captions?.[network] ?? occ.caption
          if (!captionText) { unconfirmedNetworks.push(network); continue }

          const params = new URLSearchParams({
            from: windowStart, to: windowEnd, timezone: 'America/New_York', userId, blogId: String(blogId)
          })
          const res = await fetch(`https://app.metricool.com/api/v2/analytics/posts/${network}?${params}`, {
            headers: { 'X-Mc-Auth': process.env.METRICOOL_API_TOKEN }
          })
          if (!res.ok) { unconfirmedNetworks.push(network); continue }
          const analyticsData = await res.json()
          const posts = Array.isArray(analyticsData) ? analyticsData : (analyticsData.data || [])
          // Field name for caption text varies across platforms and
          // wasn't confirmed for every one of them individually -- trying
          // the couple of plausible names rather than picking one is safe
          // here either way, since the failure mode of NOT matching is
          // just flagging something for review, not falsely trusting it.
          const snippet = captionText.slice(0, 40)
          const found = posts.some(p => [p.content, p.text, p.caption].some(v => (v || '').includes(snippet)))
          if (!found) unconfirmedNetworks.push(network)
        }
        ...
        if (unconfirmedNetworks.length === 0) {
          ...
          await supabase.from('schedule_drafts').update({
            status: 'failed',
            schedule_error: `Metricool reported published, but no matching post found in real analytics for: ${unconfirmedNetworks.join(', ')}`
          }).eq('id', occ.id)
        }
      } catch (err) {
